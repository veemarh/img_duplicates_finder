...
def get_hash(img, method = 'aHash', hash_size = 16):
    match method:
        case 'aHash':
...
def get_difference(checked_hash, curr_hash, hash_size):
    hamming_distance = checked_hash - curr_hash
    return hamming_distance / (hash_size**2) * 100
    
# search for duplicates in the source folder
def find_duplicates_use_hash(input_folder, duplicates_folder, hash_size = 16, perc = 100, method = 'aHash'):
    ...
    while check_i < len(images):
        if images[check_i] is not None:
            ...
            checked_hash = get_hash(checked_img, method, hash_size)
        ...
        while curr_i < len(images):
            if (check_i != curr_i) and (images[curr_i] is not None):
                ...
                curr_hash = get_hash(curr_img, method, hash_size)
